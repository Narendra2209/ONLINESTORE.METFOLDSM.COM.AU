...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const renderPrice = () => {
      ...
      if (product.priceRange.min === product.priceRange.max) {
        return (
          <span className="text-lg font-bold text-steel-900">
            ...
            {product.pricingModel === 'per_metre' && <span className="text-sm font-normal text-steel-500"> /m</span>}
          </span>
        );
      }
      return (
    ...
    return <span className="text-sm text-steel-500">Price on selection</span>;
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        {product.category && (
          <span className="text-[11px] font-medium uppercase tracking-wider text-steel-400 mb-1">
            {product.category.name}
          </span>
        )}

        <h3 className="text-base font-bold text-steel-900 group-hover:text-brand-600 transition-colors duration-200 line-clamp-2 leading-snug">
          {product.name}
        </h3>

        {product.shortDescription && (
          <p className="mt-1.5 text-xs text-steel-500 line-clamp-2 leading-relaxed">
            {product.shortDescription}
          </p>
        )}

        <div className="mt-auto pt-3 border-t border-steel-50">
          {renderPrice()}

          <div className="mt-2 flex items-center justify-between">
            {product.type === 'configurable' && !isQuoteOnly ? (
              <span className="text-[11px] text-steel-500 font-medium">Multiple options</span>
            ) : null}
            <span className="text-xs font-semibold text-brand-600 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-x-2 group-hover:translate-x-0">
